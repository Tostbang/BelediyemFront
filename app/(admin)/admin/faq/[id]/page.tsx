...
import { getFAQByIdAdmin } from '@/app/actions';
...
export default async function Page({
    ...
    if (id) {
        try {
            response = await getFAQByIdAdmin(id);
            if (response.success) {
                detail = response.data;
                if (detail?.code === 'NOT_FOUND' || detail?.code === '400' || detail?.code === '404') {
    ...
    const breadcrumb = [
        { label: 'SSS Listesi', href: '/admin/faq' },
